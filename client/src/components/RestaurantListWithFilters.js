...
const RestaurantListWithFilters = (props) => {
    ...
    const handleFilter = (filterValue) => {
        if (filterValue === "top") {
            ...
            const setor15Restro = restaurantsList.filter((restaurant) =>
                restaurant.areaName.toLowerCase().includes(filterValue.toLowerCase())
            );
    ...
    const handleSearch = () => {
        const searchRestro = restaurantsList.filter((restaurant) =>
    ...
    return restaurantsList.length == 0 ? (
        ...
        <>
            <h2 className="text-3xl font-semibold capitalize pb-3">{props.title}</h2>
            <div className="grid grid-cols-4 lg:grid-cols-10 py-3">
                <div
                    className="flex justify-left gap-1 items-center hover:bg-lime-600 py-2 px-3 hover:text-white cursor-pointer rounded-lg"
                ...
                <div
                    className="col-span-2 lg:col-span-1 flex justify-left gap-1 items-center hover:bg-lime-600 py-2 px-3 hover:text-white cursor-pointer rounded-lg "
